...
- Name: auth\token-persistence.spec.ts >> Token Persistence E2E >> should retain user authentication state across browser reloads
- Location: e2e\auth\token-persistence.spec.ts:4:3
...
```ts
  ...
  2  | 
  3  | test.describe('Token Persistence E2E', () => {
  4  |   test('should retain user authentication state across browser reloads', async ({ page, donorSession }) => {
> 5  |     await page.goto('/donor');
     |                ^ Error: page.goto: NS_ERROR_CONNECTION_REFUSED
  6  |     await expect(page).toHaveURL(/\/donor/);
  7  | 
  8  |     await page.reload();
  9  |     await expect(page).toHaveURL(/\/donor/);
  10 |   });
  11 | });
  12 | 
```